...
df_phono_freq1 <- read.csv("https://raw.githubusercontent.com//MatthewPaulSu/MACSFeD_data/main/Data/Literature_database_Phonotaxis_Frequency.csv")
df_wing_length1 <- read.csv("https://raw.githubusercontent.com//MatthewPaulSu/MACSFeD_data/main/Data/Literature_database_Wing_Length.csv")
...
df_wing_length <- df_wing_length1
...
df_wing_length$Tethered_Free <- rep("NA", length(df_wing_length$Group))
...
df_wing_length$Stimulus <- rep("NA", length(df_wing_length$Group))
...
df_wing_length$Mechanical_State <- rep("NA", length(df_wing_length$Group))

df_swarm$Larvae_diet <- rep("NA", length(df_swarm$Group))
df_WBF$Larvae_diet <- rep("NA", length(df_WBF$Group))
df_MT$Larvae_diet <- rep("NA", length(df_MT$Group))
df_ET$Larvae_diet <- rep("NA", length(df_ET$Group))
df_phono_all$Larvae_diet <- rep("NA", length(df_phono_all$Group))
df_phono_freq$Larvae_diet <- rep("NA", length(df_phono_freq$Group))
...
df_wing_length$'Wing Length reports' <- rep("One",length(df_wing_length$Group))

colnames(df_wing_length)[15] <- "Temperature"
...
df <- full_join(x=df, y=df_wing_length)[]

colnames(df)[12] <- "Wing Beat Frequency (Hz)"
colnames(df)[22] <- "Larvae_diet"
colnames(df)[23] <- "Wing Beat Frequency reports"
colnames(df)[26] <- 'Swarming duration (min)'
colnames(df)[27] <- 'Start (relative to sunrise)'
colnames(df)[28] <- 'Start (relative to sunset)'
colnames(df)[30] <- 'Males collected (field)'
colnames(df)[31] <- 'Swarm size by eye (field)'
colnames(df)[32] <- 'Height (m above ground)'
colnames(df)[33] <- "Swarming reports"
colnames(df)[38] <- "Mechanical Tuning Frequency (Hz)"
colnames(df)[44] <- "Mechanical Tuning reports"
colnames(df)[50] <- "Electrical Tuning Frequency (Hz)"
colnames(df)[52] <- "Electrical Tuning reports"
colnames(df)[67] <- "Phonotaxis reports"
colnames(df)[73] <- "Normalised frequency response"
colnames(df)[76] <- "Wing Length (mm)"
colnames(df)[78] <- "Rearing Temperature"

df <- df %>% relocate(Larvae_diet, .after=Rearing_density)
...
df_WL <- df[!is.na(df$'Wing Length reports'), c(1,4,9,10,11,12,18,25,26,27,30,31,14,7,20,21,75,77,79)]
df_WL$Type <- rep("Wing Length reports",length(df_WL$Group))
colnames(df_WL)[6] <- "Frequency (Hz)"

df_report <- rbind(df_SR,df_WBFR,df_MTR,df_ETR,df_PR,df_WL)
...
ui <- fluidPage(tags$head(
  tags$style(HTML("
    ...
    column(width = 3,
           wellPanel( 
             conditionalPanel(
               condition = "input.features == 'Wing Beat Frequency reports'||input.features == 'Wing Beat Frequency (Hz)'||input.features == 'Mechanical Tuning reports' ||input.features == 'Mechanical Tuning Frequency (Hz)'||input.features == 'Electrical Tuning reports' ||input.features == 'Electrical Tuning Frequency (Hz)'||input.features == 'Phonotaxis reports'||input.features == 'Wing Length reports'||input.features == 'Wing Length (mm)'", 
             ...
             conditionalPanel(
               condition = "input.features == 'Wing Length (mm)'", 
               uiOutput("input_diet", inline = TRUE), width = "100%"), 
             conditionalPanel(
               condition = "input.features == 'Wing Beat Frequency (Hz)' || input.features == 'Mechanical Tuning Frequency (Hz)'||input.features == 'Electrical Tuning Frequency (Hz)'||input.features == 'Wing Length (mm)'", 
...
server <- function(input, output, session) {
 ...
 #     pull(unique(.data$Peer_reviewed))
  ...
  output$input_species <- renderUI({
    ...
    pickerInput(
      ...
      choices = list(
        ...
        Coquillettidia = as.list(sort(unique(df.reactive()$Group[df.reactive()$Genus == "Coquillettidia"]))),
  ...
  #processing input genus to give an output displaying input WBF recording type
  ...
  #processing input genus to give an output displaying input larval diet
  output$input_diet <- renderUI({
    
    pickerInput(
      inputId = "Larvae_diet", 
      choices = c("Single diet","Multiple diets - Low","Multiple diets - Standard","Multiple diets - High"),
      #selected = unique(selected_tether),
      selected = c("Single diet","Multiple diets - Standard"),
      options = list(
        title = "Select larval diet:"), 
      multiple = TRUE,
      width = '100%'
    )
  ...
  plotInput <- reactive({
    ...
    if (selected_feature == "Wing Length (mm)"){
      number_sig <- 2}
    else{
      number_sig <- 0
    }
    ...
    }  else if (selected_feature == "Wing Length reports") {
      Plot_title <- "Number of reports per species containing information on adult wing length"
    ...
    } else if (selected_feature == "Wing Length (mm)")  {
      Plot_title <- "Estimated length of adult wings in mm"
    }   
    ...
    suppressWarnings(if(selected_feature == "Normalised frequency response"){
      ...
      df1 <- selected_data[!is.na(selected_data$'Wing Length reports'), ]
      df1 <- df1 %>%  group_by(Group, Sex, Genus, Species, Paper) %>% tally()  
      df1 <- df1 %>%  group_by(Group, Sex, Genus, Species) %>% tally() 
      p <- ggplot(data=df1, aes(x=Group, y=n, fill=Group,customdata = paste0(Sex,"__",Group))) +
        geom_bar(stat="identity")  + 
      ...
      ggplot(selected_data, aes(x = Sex, y = .data[[selected_feature]],color = Group,customdata = paste0(Sex,"__",Group,"__",Link,"__",.data[[selected_feature]]))) +
        ...
        stat_summary(inherit.aes = FALSE,aes(x = Sex, y = .data[[selected_feature]],label = round(after_stat(y), number_sig)),fun = median, geom = "text", color="black", fontface = "bold", size=5.5) +
        labs(
          x = "",
          y = input$features,
          title = Plot_title) +
    ...
    }else if (input$Facet_Option == "By temperature" & selected_feature == "Wing Length (mm)"){
      selected_data <- selected_data[selected_data$'Rearing Temperature' > 0,]
      ggplot(selected_data, aes(x = Temperature, y = .data[[selected_feature]],color = Group,customdata = paste0(Sex,"__",Group,"__",Link,"__",.data[[selected_feature]]))) +
        ...
        stat_regline_equation(inherit.aes = FALSE,aes(x = Temperature, y = .data[[selected_feature]],label =  paste0(gsub(" ","",str_sub(..eq.label..,start = -19,end =-12))," mm/°C")),label.x.npc = "centre", label.y.npc = "top") +
        labs(
          x = "Temperature (°C)",
          y = input$features,
          title = Plot_title) +
      ...
      geom_jitter(aes(text = Paper),size=4.5, alpha=0.25)+
      stat_summary(inherit.aes = FALSE,aes(x = Group, y = .data[[selected_feature]],label = round(after_stat(y), number_sig)),fun = median, geom = "text", color="black", fontface = "bold", size=5.5) +
      labs(
            x = "",
           y = input$features,
           title = Plot_title) +
  ...
  output$date <- renderText({"Version: 7th Jan 2025"})
  ...
  output$downloadAllData <- downloadHandler(
    ...
    content = function(file) {
      dataset_list <- list("Swarming" = df_swarm1, "WBF" = df_WBF1,"Mechanical Tuning" = df_MT1, "Electrical Tuning" = df_ET1,"Phonotaxis all" = df_phono_all1, "Phonotaxis frequency" = df_phono_freq1, "Wing length" = df_wing_length1)
  ...
  output$myTable <- DT::renderDataTable({
    ...
    }else if (input$features == 'Wing Length (mm)'){
      df_report <- df_report[df_report$Type == "Wing Length reports",]
    }
    if (input$features == 'Swarming reports'|| input$features == 'Phonotaxis reports'){
     datatable(unique(df_report[sprintf("<i><b>%s</i></b>", df_report$Group) %in% strsplit(click_string,"__")[[1]][2] & df_report$Type %in% input$features & df_report$Sex %in% strsplit(click_string,"__")[[1]][1] & df_report$Experiment_location %in% input$Experiment_location & df_report$Peer_reviewed %in% input$Peer_reviewed,-c(6:13,15:20)]), extensions = 'Buttons', options = list(dom = 'Blrtip',buttons = c('copy', 'excel')),class = "display")   %>%
        formatStyle("Group",
                    fontStyle = "italic")
    } else if (input$features == 'Wing Beat Frequency reports'){
      datatable(unique(df_report[sprintf("<i><b>%s</i></b>", df_report$Group) %in% strsplit(click_string,"__")[[1]][2] & df_report$Type %in% input$features & df_report$Sex %in% strsplit(click_string,"__")[[1]][1] & df_report$Experiment_location %in% input$Experiment_location & df_report$Tethered_Free %in% input$Tethered_Free & df_report$Peer_reviewed %in% input$Peer_reviewed,-c(6:12,15:20)]), extensions = 'Buttons', options = list(dom = 'Blrtip',buttons = c('copy', 'excel')),class = "display")   %>%
        formatStyle("Group",
                    fontStyle = "italic")
    } else if (input$features == 'Mechanical Tuning reports'||input$features == 'Electrical Tuning reports'){
      datatable(unique(df_report[sprintf("<i><b>%s</i></b>", df_report$Group) %in% strsplit(click_string,"__")[[1]][2] & df_report$Type %in% input$features & df_report$Sex %in% strsplit(click_string,"__")[[1]][1] & df_report$Experiment_location %in% input$Experiment_location & df_report$Peer_reviewed %in% input$Peer_reviewed & df_report$Stimulus %in% input$Stimulus & df_report$Mechanical_State %in% input$Mechanical_State,-c(6:13,17:20)]), extensions = 'Buttons', options = list(dom = 'Blrtip',buttons = c('copy', 'excel')),class = "display")   %>%
        formatStyle("Group",
                    fontStyle = "italic")      
    } else if (input$features == 'Wing Length reports'){
      datatable(unique(df_report[sprintf("<i><b>%s</i></b>", df_report$Group) %in% strsplit(click_string,"__")[[1]][2] & df_report$Type %in% input$features & df_report$Sex %in% strsplit(click_string,"__")[[1]][1] & df_report$Experiment_location %in% input$Experiment_location & df_report$Peer_reviewed %in% input$Peer_reviewed & df_report$Larvae_diet %in% input$Larvae_diet,-c(6:13,15:17)]), extensions = 'Buttons', options = list(dom = 'Blrtip',buttons = c('copy', 'excel')),class = "display")   %>%
        formatStyle("Group",
                    fontStyle = "italic")
    }else if (input$features == 'Swarming duration (min)'){
      datatable(unique(df_report[sprintf("<i><b>%s</i></b>", df_report$Group) %in% strsplit(click_string,"__")[[1]][2] & df_report$'Swarming duration (min)' %in% strsplit(click_string,"__")[[1]][4],-c(6:7,9:13,15:20)]), extensions = 'Buttons', options = list(dom = 'Blrtip',buttons = c('copy', 'excel')),class = "display")   %>%
        formatStyle("Group",
                    fontStyle = "italic")
    }else if (input$features == 'Start (relative to sunrise)'){
      datatable(unique(df_report[sprintf("<i><b>%s</i></b>", df_report$Group) %in% strsplit(click_string,"__")[[1]][2] & df_report$'Start (relative to sunrise)' %in% strsplit(click_string,"__")[[1]][4],-c(6:8,10:13,15:20)]), extensions = 'Buttons', options = list(dom = 'Blrtip',buttons = c('copy', 'excel')),class = "display")   %>%
        formatStyle("Group",
                    fontStyle = "italic")
    }else if (input$features == 'Start (relative to sunset)'){
      datatable(unique(df_report[sprintf("<i><b>%s</i></b>", df_report$Group) %in% strsplit(click_string,"__")[[1]][2] & df_report$'Start (relative to sunset)' %in% strsplit(click_string,"__")[[1]][4],-c(6:9,11:13,15:20)]), extensions = 'Buttons', options = list(dom = 'Blrtip',buttons = c('copy', 'excel')),class = "display")   %>%
        formatStyle("Group",
                    fontStyle = "italic")
    }else if (input$features == 'Swarm size by eye (field)'){
      datatable(unique(df_report[sprintf("<i><b>%s</i></b>", df_report$Group) %in% strsplit(click_string,"__")[[1]][2] & df_report$'Swarm size by eye (field)' %in% strsplit(click_string,"__")[[1]][4],-c(6:10,12:13,15:20)]), extensions = 'Buttons', options = list(dom = 'Blrtip',buttons = c('copy', 'excel')),class = "display")   %>%
        formatStyle("Group",
                    fontStyle = "italic")
    }else if (input$features == 'Height (m above ground)'){
      datatable(unique(df_report[sprintf("<i><b>%s</i></b>", df_report$Group) %in% strsplit(click_string,"__")[[1]][2] & df_report$'Height (m above ground)' %in% strsplit(click_string,"__")[[1]][4],-c(6:11,13,15:20)]), extensions = 'Buttons', options = list(dom = 'Blrtip',buttons = c('copy', 'excel')),class = "display")   %>%
        formatStyle("Group",
                    fontStyle = "italic")
    } else if (input$features == 'Wing Length (mm)'){
      datatable(unique(df_report[sprintf("<i><b>%s</i></b>", df_report$Group) %in% strsplit(click_string,"__")[[1]][2] & df_report$Larvae_diet %in% input$Larvae_diet & df_report$'Wing Length (mm)' %in% strsplit(click_string,"__")[[1]][4],-c(6:13,15:17)]), extensions = 'Buttons', options = list(dom = 'Blrtip',buttons = c('copy', 'excel')),class = "display")   %>%
        formatStyle("Group",
                    fontStyle = "italic")
    } else if (input$features == 'Normalised frequency response') {
      datatable(df_report[df_report$Link %in% strsplit(click_string,"__")[[1]][3] & sprintf("<i><b>%s</i></b>", df_report$Group) %in% strsplit(click_string,"__")[[1]][2] & df_report$Sex %in% strsplit(click_string,"__")[[1]][1],-c(6:13,15:20)], extensions = 'Buttons', options = list(dom = 'Blrtip',buttons = c('copy', 'excel')),class = "display")    %>%
        formatStyle("Group",
                    fontStyle = "italic")
      } else {datatable(df_report[df_report$Link %in% strsplit(click_string,"__")[[1]][3] & sprintf("<i><b>%s</i></b>", df_report$Group) %in% strsplit(click_string,"__")[[1]][2] & df_report$Sex %in% strsplit(click_string,"__")[[1]][1] & df_report$'Frequency (Hz)' %in% strsplit(click_string,"__")[[1]][4],-c(8:13,15:20)], extensions = 'Buttons', options = list(dom = 'Blrtip',buttons = c('copy', 'excel')),class = "display")    %>%
        formatStyle("Group",
                    fontStyle = "italic")}
     })
